<#
    Script: Create user Domain Admin + DNS Admin
...
do {
    $UserName = Read-Host "Enter the username you want to create"
    if ([string]::IsNullOrWhiteSpace($UserName)) {
        Write-Host "Username cannot be empty. Please enter a valid username." -ForegroundColor Red
    }
} until ($UserName -and $UserName.Trim() -ne "")

# Ask for secure password
...
Write-Host "User '$UserName' added to Domain Admins." -ForegroundColor Green
...
<# ======================================================================
       DOMAIN CONTROLLER HARDENING SCRIPT
       SAFEST VERSION – NO LDAP HARDENING – NETWORK-SCOPE FIREWALL RULES
       Author: Cesar
====================================================================== #>

Write-Host "=== DOMAIN HARDENING SCRIPT STARTED ===" -ForegroundColor Cyan

# ==========================================================
#  SECTION 1 – VARIABLES (SAFE VERSION)
# ==========================================================
$NetworkScope = "192.168.220.0/24"
Write-Host "[i] Using NetworkScope: $NetworkScope" -ForegroundColor Cyan

# No machine-specific assignments
$Machines = @{}

# ==========================================================
#  SECTION X – PORT DEFINITIONS
# ==========================================================

# AD Essential TCP Ports
$NetworkPortsTCP = @(
    88,     # Kerberos
    389,    # LDAP
    636,    # LDAPS
...
# Strict Ports (Only Allowed Inside Internal Network)
$StrictTcpPorts = @(
    22,     # SSH (Linux)
...
# ==========================================================
#  SECTION 2 – NETWORK-WIDE LDAP / KERBEROS RULES
# ==========================================================
Write-Host "`n[1] Applying Network-Wide Kerberos/LDAP rules..." -ForegroundColor Yellow

foreach ($p in $NetworkPortsTCP) {
    New-NetFirewallRule -DisplayName "Allow NET TCP $p (Kerberos/LDAP)" `
        -Direction Inbound -Protocol TCP -LocalPort $p `
        -RemoteAddress $NetworkScope -Profile Domain -Action Allow `
        -ErrorAction SilentlyContinue | Out-Null
}

foreach ($p in $NetworkPortsUDP) {
    New-NetFirewallRule -DisplayName "Allow NET UDP $p (Kerberos/LDAP)" `
        -Direction Inbound -Protocol UDP -LocalPort $p `
        -RemoteAddress $NetworkScope -Profile Domain -Action Allow `
        -ErrorAction SilentlyContinue | Out-Null
}

Write-Host "[OK] Kerberos/LDAP Allow Rules Applied." -ForegroundColor Green

# ==========================================================
#  SECTION 2.5 – GLOBAL RDP ACCESS
# ==========================================================
Write-Host "`n[1.5] Applying RDP Rules (TCP/UDP 3389 from ANY IP)..." -ForegroundColor Yellow

New-NetFirewallRule -DisplayName "Allow RDP TCP 3389" -Direction Inbound `
    -Protocol TCP -LocalPort 3389 -RemoteAddress Any -Profile Domain `
    -Action Allow -ErrorAction SilentlyContinue | Out-Null

New-NetFirewallRule -DisplayName "Allow RDP UDP 3389" -Direction Inbound `
    -Protocol UDP -LocalPort 3389 -RemoteAddress Any -Profile Domain `
    -Action Allow -ErrorAction SilentlyContinue | Out-Null

Write-Host "[OK] RDP Rules Applied." -ForegroundColor Green

# ==========================================================
#  SECTION 2.8 – ALLOW NTP (UDP 123)
# ==========================================================
Write-Host "`n[2.8] Applying NTP Rule..." -ForegroundColor Yellow

New-NetFirewallRule -DisplayName "Allow NTP UDP 123" `
    -Direction Inbound -Protocol UDP -LocalPort 123 `
    -RemoteAddress Any -Profile Domain -Action Allow `
    -ErrorAction SilentlyContinue | Out-Null

Write-Host "[OK] NTP Rule Applied." -ForegroundColor Green

# ==========================================================
#  SECTION 2.85 – CONDITIONAL DNS RULES (SCORING MODE)
# ==========================================================

Write-Host "`n[2.85] DNS Firewall Rules Setup..." -ForegroundColor Yellow

# Ask user if DNS is being scored
$dnsScored = Read-Host "Is DNS being scored by the competition? (Y/N)"

if ($dnsScored -match '^[Yy]') {
    Write-Host "DNS is being scored – allowing DNS from ANY IP..." -ForegroundColor Green

    # Allow DNS UDP from ANY
    New-NetFirewallRule -DisplayName "Allow DNS UDP 53 (Global - Scored)" `
        -Direction Inbound -Protocol UDP -LocalPort 53 `
        -RemoteAddress Any -Action Allow -Profile Domain `
        -ErrorAction SilentlyContinue | Out-Null

    # Allow DNS TCP from ANY
    New-NetFirewallRule -DisplayName "Allow DNS TCP 53 (Global - Scored)" `
        -Direction Inbound -Protocol TCP -LocalPort 53 `
        -RemoteAddress Any -Action Allow -Profile Domain `
        -ErrorAction SilentlyContinue | Out-Null

    Write-Host "[OK] DNS Rules Applied for Scoring Mode (ANY IP Allowed)" -ForegroundColor Green
}
else {
    Write-Host "DNS is NOT being scored – restricting DNS to internal scope $NetworkScope..." -ForegroundColor Yellow

    # Allow DNS UDP from NetworkScope
    New-NetFirewallRule -DisplayName "Allow DNS UDP 53 (Internal Only)" `
        -Direction Inbound -Protocol UDP -LocalPort 53 `
        -RemoteAddress $NetworkScope -Action Allow -Profile Domain `
        -ErrorAction SilentlyContinue | Out-Null

    # Allow DNS TCP from NetworkScope
    New-NetFirewallRule -DisplayName "Allow DNS TCP 53 (Internal Only)" `
        -Direction Inbound -Protocol TCP -LocalPort 53 `
        -RemoteAddress $NetworkScope -Action Allow -Profile Domain `
        -ErrorAction SilentlyContinue | Out-Null

    Write-Host "[OK] DNS Rules Applied for Internal Network Only" -ForegroundColor Green
}

# ==========================================================
#  SECTION 2.9 – ALLOW ICMP (PING)
# ==========================================================
Write-Host "`n[2.9] Applying ICMP Rules..." -ForegroundColor Yellow

New-NetFirewallRule -DisplayName "Allow ICMPv4 Echo Request" `
    -Direction Inbound -Protocol ICMPv4 -IcmpType 8 -RemoteAddress Any `
    -Action Allow -Profile Domain -ErrorAction SilentlyContinue | Out-Null

New-NetFirewallRule -DisplayName "Allow ICMPv4 Echo Reply" `
    -Direction Inbound -Protocol ICMPv4 -IcmpType 0 -RemoteAddress Any `
    -Action Allow -Profile Domain -ErrorAction SilentlyContinue | Out-Null

Write-Host "[OK] ICMP Rules Applied." -ForegroundColor Green

# ==========================================================
#  SECTION 3 – STRICT PORTS (NETWORK-WIDE)
# ==========================================================
Write-Host "`n[2] Applying Strict Internal Network Rules..." -ForegroundColor Yellow

foreach ($p in $StrictTcpPorts) {
    New-NetFirewallRule -DisplayName "Allow Internal TCP $p" `
        -Direction Inbound -Protocol TCP -LocalPort $p `
        -RemoteAddress $NetworkScope -Action Allow -Profile Domain `
        -ErrorAction SilentlyContinue | Out-Null
}
...
foreach ($p in $StrictUdpPorts) {
    New-NetFirewallRule -DisplayName "Allow Internal UDP $p" `
        -Direction Inbound -Protocol UDP -LocalPort $p `
        -RemoteAddress $NetworkScope -Action Allow -Profile Domain `
        -ErrorAction SilentlyContinue | Out-Null
}

New-NetFirewallRule -DisplayName "Allow Internal RPC High Ports" `
    -Direction Inbound -Protocol TCP -LocalPort "$DynStart-$DynEnd" `
    -RemoteAddress $NetworkScope -Action Allow -Profile Domain `
    -ErrorAction SilentlyContinue | Out-Null

Write-Host "[OK] Strict Internal Network Rules Applied." -ForegroundColor Green

# =====================================================================
#   SECTION 4 - DOMAIN CONTROLLER HARDENING – SAFE VERSION
# =====================================================================
...
try {
    ...
    Write-Host "[ERROR] Cannot detect AD domain." -ForegroundColor Red
    exit
}
...
if (-not $gpo) {
    $gpo = New-GPO -Name $GpoName
    Write-Host "  → GPO Created: $GpoName" -ForegroundColor Cyan
} else {
...
New-GPLink -Name $GpoName -Target $DC_OU -LinkEnabled Yes `
    -ErrorAction SilentlyContinue | Out-Null
Set-GPLink -Name $GpoName -Target $DC_OU -Order 1

Write-Host "[OK] GPO Linked to Domain Controllers OU" -ForegroundColor Green

# ==========================================================
# FIREWALL PROFILE HARDENING
# ==========================================================
...
# ==========================================================
# SMB HARDENING (SAFE – DOES NOT TOUCH LDAP)
# ==========================================================
...
Set-GPRegistryValue -Name $GpoName -Key "HKLM\System\CurrentControlSet\Services\LanmanServer\Parameters" `
    -ValueName "SMB2" -Type DWord -Value 1
...
# Enable server-side SMB3 encryption
Set-SmbServerConfiguration -EncryptData $true -Force

Write-Host "[OK] SMB hardened." -ForegroundColor Green

# ==========================================================
# LEGACY PROTOCOL DISABLE (SAFE)
# ==========================================================
...
Set-GPRegistryValue -Name $GpoName `
    -Key "HKLM\System\CurrentControlSet\Services\LanmanWorkstation\Parameters" `
    -ValueName "AllowInsecureGuestAuth" -Type DWord -Value 0
...
# ==========================================================
# ZEROLOGON PROTECTION (SAFE)
# ==========================================================
Write-Host "`n[4.8] Enforcing ZeroLogon Protections..." -ForegroundColor Cyan
...
Write-Host "[✔] ZeroLogon Mitigations Applied." -ForegroundColor Green

# ==========================================================
# FINAL APPLY
# ==========================================================
Write-Host "`n[+] Applying GPO…" -ForegroundColor Cyan
gpupdate /force

Write-Host "`n🔥 DOMAIN HARDENING COMPLETE – SAFE VERSION 🔥" -ForegroundColor Yellow
